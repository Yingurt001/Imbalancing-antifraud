...
def build_test_data(grouped_data, master_data_size, split_ratio, stats_accumulator):
    ...
    - split_ratio: 欠采样比例（非违约 : 违约），或 "original"
    ...
    if split_ratio == "original":
        ...
        print("✅ Using original ratio (no undersampling).")
    else:
...
if __name__ == "__main__":
    # 原始数据来源
    input_files = [
        "data/processed_data/historical_data_time_2021Q1.csv",
        "data/processed_data/historical_data_time_2021Q2.csv",
        "data/processed_data/historical_data_time_2021Q3.csv",
        "data/processed_data/historical_data_time_2021Q4.csv"
    ]

    # 设置不同的主数据长度和采样比例
    master_data_sizes = [9, 12, 15, 18, 21, 24, 27, 30, 33, 36]
    split_ratios = [1, 2, 5, 10, "original"]

    # 汇总统计信息保存路径
    stats_csv_path = "E2_test_data_summary_2021.csv"

    # 如果旧文件存在，先删除，保证新的干净
    if os.path.exists(stats_csv_path):
        os.remove(stats_csv_path)
    first_write = True  # 控制是否写表头
    ...
    for master_data_size in master_data_sizes:
        for split_ratio in split_ratios:
            ...
            if all_X and all_Y:
                ...
                # 按参数命名保存路径
                if split_ratio == "original":
                    output_dir = f"data/test_merged_{year_str}_master{master_data_size}_ratio_original"
                    ratio_str = "original"
                else:
                ...
                df_row.to_csv(
                    stats_csv_path,
                    mode='a',
                    header=first_write,  # 只有第一次写入才写表头
                    index=False
                )
                first_write = False
