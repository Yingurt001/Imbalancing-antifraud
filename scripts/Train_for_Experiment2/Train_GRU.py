...
def masked_GRU_model(input_shape):
    ...
    gru_out2 = GRU(64, return_sequences=True)(gru_out)
    flattened_out = Flatten()(gru_out2)
...
def evaluate_model(interval, test_X, test_Y, run):
    try:
        # 加载每次运行中验证集上表现最好的模型
        model_path = f'实验二models/GRU_best_model_interval_{interval}_run_{run}.h5'
    ...
    auc = roc_auc_score(test_Y, yhat_probs)  # 计算 AUC
...
if __name__ == "__main__":
    results = []
    csv_file_path = 'AUC_E2_GRU_model_results_part_3.csv'  # 保存结果的CSV文件路径
    intervals = [5,6, 7, 8,9]  # 预定义的 interval 值
    ...
    for interval in intervals:
        ...
        for run in range(1, 21):  # 每个 interval 运行 20 次
            ...
            try:
                ...
                model_save_path = f'实验二models/GRU_best_model_interval_{interval}_run_{run}.h5'
