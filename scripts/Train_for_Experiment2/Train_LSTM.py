...
def evaluate_model(interval, test_X, test_Y, run):
    try:
        # 加载每次运行中验证集上表现最好的模型
        model_path = f'实验二models/LSTM_best_model_interval_{interval}_run_{run}.h5'
...
if __name__ == "__main__":
    results = []
    csv_file_path = 'E2_LSTM_model_results.csv'  # 保存结果的CSV文件路径
    ...
    for interval in intervals:
        ...
        for run in range(1, 21):  # 每个 interval 运行 20 次
            ...
            try:
                ...
                model_save_path = f'实验二models/LSTM_best_model_interval_{interval}_run_{run}.h5'
