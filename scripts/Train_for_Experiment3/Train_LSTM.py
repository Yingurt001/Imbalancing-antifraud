...
def evaluate_model(master_data_size, test_X, test_Y, run):
    try:
        # 加载每次运行中验证集上表现最好的模型
        model_path = f'实验三models/LSTM_best_model_master_data_size_{master_data_size}_run_{run}.h5'
...
# 保存结果到 CSV
def save_results_to_csv(    esults, filepath):
...
if __name__ == "__main__":
    results = []
    csv_file_path = 'E3_LSTM_model_results_part3.csv'  # 保存结果的CSV文件路径
      # 每个 master_data_size 运行 20 次
    master_data_sizes = [24]  # 预定义的 master_data_size 值
    ...
    for master_data_size in master_data_sizes:
        ...
        for run in range(1,  21):  # 每个 master_data_size 运行 20 次
            ...
            try:
                ...
                model_save_path = f'实验三models/LSTM_best_model_master_data_size_{master_data_size}_run_{run}.h5'
