...
def build_test_data(grouped_data, interval):
    test_data_X = []
    test_data_Y = []
    MASTER_DATA_SIZE = 24
    predict_month = 3
    ...
    for name, group in grouped_data:
        ...
        if group_size >= MASTER_DATA_SIZE:
            # 提取特征(X)和标签(Y)
            test_X = group.iloc[0:MASTER_DATA_SIZE-interval-predict_month-1, 1:-1]  # 选择相关特征列
...
def process_interval(interval):
    ...
    # 构建测试数据
    test_X, test_Y = build_test_data(grouped_data, interval)
    
    # 保存数据到不同的目录
    output_dir = f'data/npy_test_interval_21年_{interval}'
...
if __name__ == "__main__":
    ...
    if not os.path.exists(input_file):
        ...
        # 定义不同的 interval 值
        intervals = [4, 5, 7,8]  # 例如，使用三个不同的 interval 进行测试
