...
def build_test_data(grouped_data, master_data_size):
    test_data_X = []
    test_data_Y = []
    predict_month = 3
    SPLIT = 3  # 控制未违约样本的比例
    ...
    for name, group in grouped_data:
        ...
        # 只取长度>=master_data_size的贷款组
        group_size = len(group)
        if group_size >= master_data_size:
            # 提取特征(X)和标签(Y)
            test_X = group.iloc[0:master_data_size-predict_month-1, 1:-1]  # 选择相关特征列
            test_Y = group.iloc[master_data_size-predict_month-1:master_data_size-1, -1]  # 选择标签列
...
# 多线程处理函数
def process_master_data_size(master_data_size):
    # 加载并处理数据
    grouped_data = load_and_process_data(input_file)
    
    # 构建测试数据
    test_X, test_Y = build_test_data(grouped_data, master_data_size)
    
    # 保存数据到不同的目录
    output_dir = f'data/npy_test_master_data_size_21年_{master_data_size}'
    save_data(test_X, test_Y, output_dir)

if __name__ == "__main__":
    # 设置输入文件路径
    input_file = "data/processed_data/historical_data_time_2021Q1.csv"
    
    # 检查文件是否存在
    if not os.path.exists(input_file):
        print(f"Error: The file '{input_file}' does not exist. Please check the path and try again.")
    else:
        # 定义不同的 MASTER_DATA_SIZE 值
        master_data_sizes = [15, 18, 21, 24, 27]  # 例如，使用这些不同的 MASTER_DATA_SIZE 进行测试

        # 使用 ThreadPoolExecutor 来并行处理不同的 MASTER_DATA_SIZE
        with ThreadPoolExecutor(max_workers=len(master_data_sizes)) as executor:
            executor.map(process_master_data_size, master_data_sizes)
