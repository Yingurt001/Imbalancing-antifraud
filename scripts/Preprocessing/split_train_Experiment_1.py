...
def save_data(test_X, test_Y, output_dir='data/npy_test'):
    ...
    # 保存处理后的数据
    np.save(os.path.join(output_dir, 'trainX.npy'), test_X)
    np.save(os.path.join(output_dir, 'trainY.npy'), test_Y)
    print(f"Test data and labels have been saved to '{output_dir}/TrainX.npy' and '{output_dir}/TrainY.npy'.")
